(progn
  ...
  (declare 'incf (lambda '(n) '(set n (+ n 1))))
  ((lambda nil '(progn (define 'n 1) (declare 'printc (lambda nil '(progn (print n) (print '-) (incf n)))) (declare 'reset (lambda nil '(set n 1))))))
  (declare 'print_creature (lambda '(c) '(progn (printc) (print (c 'name)) (print ':) (print (c 'health)) (terpri))))
  (declare '*baddies* (creatures (progn (print '#?) (read 20))))
  (mapcar print_creature *baddies*) (reset)
  (declare 'loop (lambda nil '(progn (define 'input (read 200))
				     (cond ((eq (car input) 'damage) (damage (nth (- (car (cdr input)) 1) *baddies*))))
				     (cond ((null (eq input 'quit)) (progn (mapcar print_creature *baddies*) (reset) (loop)))))))
  (loop)
  t)
